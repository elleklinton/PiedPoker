...
class TestStraight(TestCase):
    ...
    def test_cards_in_hand(self):
        ...
        self.assertEqual(BaseHand(a).__as_hand__(Straight).cards_in_hand, HandTestUtils.build_shorthand('6s', '5c', '4c', '3c', '2c'))
        self.assertEqual(BaseHand(a).__as_hand__(Straight).cards_not_in_hand, HandTestUtils.build_shorthand())
        ...
        self.assertEqual(BaseHand(a).__as_hand__(Straight).cards_in_hand, HandTestUtils.build_shorthand('7s', '6c', '5c', '4c', '3c'))
        self.assertEqual(BaseHand(a).__as_hand__(Straight).cards_not_in_hand, HandTestUtils.build_shorthand())
    ...
    def test_ace_low_straight(self):
        ...
        self.assertEqual(BaseHand(a).__as_hand__(Straight).cards_in_hand, HandTestUtils.build_shorthand('5s', '4c', '3s', '2d', 'ah'))
        self.assertEqual(BaseHand(a).__as_hand__(Straight).cards_not_in_hand, HandTestUtils.build_shorthand())

        # Ace straight is not highest straight
        a = HandTestUtils.build_shorthand('6h', '5s', '4c', '3s', '2d', 'ah')
        self.assertEqual(BaseHand(a).__as_hand__(Straight).cards_in_hand, HandTestUtils.build_shorthand('6h', '5s', '4c', '3s', '2d'))
        self.assertEqual(BaseHand(a).__as_hand__(Straight).cards_not_in_hand, HandTestUtils.build_shorthand())
    ...
    def test_all_possible_straights(self):
        ...
        self.assertEqual(BaseHand(a).__as_hand__(Straight).cards_in_hand, HandTestUtils.build_shorthand('6s', '5s', '4c', '3s', '2d'))
        HandTestUtils.assertGreaterThan(self, Straight, a, last)
        last = a

        a = HandTestUtils.build_shorthand('7c', '6s', '5s', '4c', '3s', '2d', 'ah')
        self.assertEqual(BaseHand(a).__as_hand__(Straight).cards_in_hand, HandTestUtils.build_shorthand('7c', '6s', '5s', '4c', '3s'))
        HandTestUtils.assertGreaterThan(self, Straight, a, last)
        last = a

        a = HandTestUtils.build_shorthand('8s', '7c', '6s', '5s', '4c', '3s', '2d', 'ah')
        self.assertEqual(BaseHand(a).__as_hand__(Straight).cards_in_hand, HandTestUtils.build_shorthand('8s', '7c', '6s', '5s', '4c'))
        HandTestUtils.assertGreaterThan(self, Straight, a, last)
        last = a

        a = HandTestUtils.build_shorthand('9c', '8s', '7c', '6s', '5s', '4c', '3s', '2d', 'ah')
        self.assertEqual(BaseHand(a).__as_hand__(Straight).cards_in_hand, HandTestUtils.build_shorthand('9c', '8s', '7c', '6s', '5s'))
        HandTestUtils.assertGreaterThan(self, Straight, a, last)
        last = a

        a = HandTestUtils.build_shorthand('10s', '9c', '8s', '7c', '6s', '5s', '4c', '3s', '2d', 'ah')
        self.assertEqual(BaseHand(a).__as_hand__(Straight).cards_in_hand, HandTestUtils.build_shorthand('10s', '9c', '8s', '7c', '6s'))
        HandTestUtils.assertGreaterThan(self, Straight, a, last)
        last = a

        a = HandTestUtils.build_shorthand('jc', '10s', '9c', '8s', '7c', '6s', '5s', '4c', '3s', '2d', 'ah')
        self.assertEqual(BaseHand(a).__as_hand__(Straight).cards_in_hand, HandTestUtils.build_shorthand('jc', '10s', '9c', '8s', '7c'))
        HandTestUtils.assertGreaterThan(self, Straight, a, last)
        last = a

        a = HandTestUtils.build_shorthand('qc', 'jc', '10s', '9c', '8s', '7c', '6s', '5s', '4c', '3s', '2d', 'ah')
        self.assertEqual(BaseHand(a).__as_hand__(Straight).cards_in_hand, HandTestUtils.build_shorthand('qc', 'jc', '10s', '9c', '8s'))
        HandTestUtils.assertGreaterThan(self, Straight, a, last)
        last = a

        a = HandTestUtils.build_shorthand('ks', 'qc', 'jc', '10s', '9c', '8s', '7c', '6s', '5s', '4c', '3s', '2d')
        self.assertEqual(BaseHand(a).__as_hand__(Straight).cards_in_hand, HandTestUtils.build_shorthand('ks', 'qc', 'jc', '10s', '9c'))
        HandTestUtils.assertGreaterThan(self, Straight, a, last)
        last = a

        a = HandTestUtils.build_shorthand('ac', 'ks', 'qc', 'jc', '10s', '9c', '8s', '7c', '6s', '5s', '4c', '3s', '2d')
        self.assertEqual(BaseHand(a).__as_hand__(Straight).cards_in_hand, HandTestUtils.build_shorthand('ac', 'ks', 'qc', 'jc', '10s'))
        HandTestUtils.assertGreaterThan(self, Straight, a, last)

    def test_non_straight(self):
        a = HandTestUtils.build_shorthand('qc', 'kc', 'ac', '2c', '3c')
        self.assertFalse(BaseHand(a).__as_hand__(Straight).is_hand)
    ...
    def test_outs(self):
        ...
        self.assertEqual(BaseHand(a).__as_hand__(Straight).__hand_outs__(set()), HandTestUtils.build_shorthand(
            'kc', 'kd', 'kh', 'ks'
        ))
